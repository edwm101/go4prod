...
export default {
  ...
  watch: {
    async p_setdata() {
      this.resetForm();
      var response = await r_getProductInfo();
      ...
      if (this.p_id != null) {
        var responseProduct = await r_getProduct(this.p_id);
  ...
  methods: {
    ...
    async saveProduct() {
      ...
      if (this.$refs.form.validate()) {
        this.progress = true;
        try {
          var response = await r_updateProduct({
            ...this.form,
            ...{ product_id }
          });
    ...
    async deleteProduct() {
      ...
      if (accept) {
        r_deleteProduct(this.p_id).then(response => {
          this.dialog = false;
