...
class Func
{
    ...
    {
        $data =  json_decode(file_get_contents(preg_replace('/\s+/', '%20',"https://maps.googleapis.com/maps/api/place/".$query."&key=".MAP_API_KEY)), false);
        return  $data;
    }
    ...
    public static function error($errorMsg = null, $reponse = 401, $status = null)
    {
    ...
    public static function setArray($array, $propertie, $value, $where = 'left')
    {
        foreach ($array as $key => $val) {
            ...
            if (!empty($prop_value)) {
                if ($where == 'left')
                    $array[$key][$propertie] = $value . $prop_value;
                else
                    $array[$key][$propertie] = $prop_value . $value;
            }
        }
    ...
    public static function slugify($string, $maxLenght = 5)
    {
        // -- Returns the slug
    ...
    {
        return self::slugify($name, $nameLength) .'-'. self::randomString($randomLength);
    }
